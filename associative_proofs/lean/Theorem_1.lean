...
def depth : NestedPair → ℕ
| Empty => 0
| Doublet _ p' => Nat.succ (depth p')
...
section TuplesNets
  ...
  def Tuple (n : ℕ) : Type :=
    match n with
    | 0 => unit
    | nat.succ n' => L × Tuple n'
    end

  /- Определение зависимого типа ассоциативной сети кортежей фиксированной длины n
  TuplesNet в данном случае является зависимым типом,
  поскольку тип Tuple n зависит от значения n.
  Зависимый тип означает, что тип зависит от значений или других типов в программе. -/
  def TuplesNet (n : ℕ) : Type := L → Tuple n
end TuplesNets
