...
Разберем основные понятия и термины, используемые в Теории Связей. 

Затем перейдем к описанию Теории Связей в рамках Теории Множеств. 

Далее рассмотрим, как Теория Связей может быть представлена в рамках Теории Типов, используя интерактивное программное средство доказательства теорем “Coq”. 
...
Теория связей основана на едином фундаментальном понятии связи.
В теории связей есть только связи, которые могут играть разные роли, такие как типы, значения, множества, элементы, функции, упорядоченные пары, узлы и ребра.

### Связь

Связь имеет ассиметричную рекурсивную структуру: связь связывает связи.

Связь описывается ачислом (ассоциативным числом), т.е. последовательностью ассоциаций.
...
Ссылка - связь в роли источника ассоциаций
Ассоциация - связь в роли сопоставления ссылки -> значению.
Значение - связь в роли значения ассоциированного со ссылкой.
Контекст - связь в роли сопоставления значения -> ссылке.
...
### Представление понятий Тоерии множеств в ассоциативной сети дуплетов

Связи хранимые АСД могут играть разные роли, позволяя описывать различные математические структуры Теории Множеств:

* упорядоченные пары ссылок
* сопоставления соответствия
* элемент множеств
* множество элементов
* БО (бинарные отношения)
* БОА (бинарное отношение агрегирования)
* ФБО (функциональное бинарное отношение) т.е. функции

#### Множество связей в роли УП может описывать состав БО как множество

**БО** - это подмножество ПП (прямого произведения) двух множеств.
Таким образом, УП (упорядоченные пары), входящие в это БО, являются элементами множества, описывающего данное отношение.
...
которое содержит сопоставления элементов множествам в виде УП (ссылка на связь в роли элемент, ссылка на связь в роли множества), и описывает состав всех множеств.

Таким образом, БОА может описывать бинарные отношения в виде множества УП вида (ссылка на связь в роли УП, ссылка на связь в роли БО),
где каждая УП сопоставляет элемент БО и бинарное отношение как множество УП.

#### Множество связей в роли ассоциации, т.е. сопоставления прообраза образу может описывать ФБО

ФБО - это такое бинарное отношение между двумя множествами,
при котором каждому элементу первого множества (прообразу) может соответствовать не больше одного элемента второго множества (образа).
Т.е. ФБО это множество УП вида (ссылка на связь в роли прообраз, ссылка на связь в роли образ).

Таким образом, БОА, описывающая множества, может описывать ФБО в виде множества УП вида (ссылка на связь в роли УП, ссылка на связь в роли ФБО),
где каждая УП сопоставляет сопоставление как элемент и ФБО множество сопоставлений.

#### Связь в роли множества может описывать множество элементов
...
Применение Теории связей позволяет нам переосмыслить традиционные математические структуры, такие как множества элементов, используя связи вместо абстрактных объектов. В таком контексте одна связь может акт как множество, собирая под собой элементы, которые определены как принадлежащие этому множеству.

Когда мы рассматриваем упорядоченные пары (УП) в рамках бинарного отношения агрегирования (БОА), такие пары состоят из ссылки на связь в роли элемента и ссылки на связь в роли множества. Эта метамодель позволяет нам представлять множества элементов таким образом, что каждый элемент может быть явно идентифицирован по своей связи.

#### Связь в роли элемента может описывать элемент множества
...
Аналогично, связь может служить в роли элемента, которая включена в упорядоченные пары БОА. Таким образом, каждая упорядоченная пара представляет собой ассоциацию между элементом и множеством, в которое он входит. 

Элемент, в данном случае, не просто абстрактная точка данных — он является частью ассоциативной сети, которая может картироваться на множество разнообразных структур в рамках Теории связей, включая и другие множества или упорядоченные пары, составляющие различные бинарные отношения.

Такое представление позволяет системе гибко реагировать на изменения и анализировать структуру данных на более глубоком уровне. Элементы не вынуждены быть статичными и могут перемещаться между множествами, изменять свои свойства и отношения с другими элементами, исходя из динамически изменяющихся правил и операций, определенных в системе.

Это подчеркивает мощь и адаптируемость Теории связей, позволяя создавать сложные и масштабируемые структуры, которые отражают как статические, так и динамические свойства данных в моделируемой системе.



#### БОА может описывать множество БО, как БО состоящее из УП вида (ссылка на связь в роли УП, ссылка на связь в роли БОА).

Если мы имеем несколько ФА, каждая из которых определяется своим ФБО,
  то один и тот же элемент может быть сопоставлен разным множествам в рамках разных ФА.

Исходя из предыдущих утверждений, УП (элемент, множество) может сама по себе рассматриваться как элемент в контексте ФА.
...
Исходя из определения ФА как множества УП, каждое такое множество можно интерпретировать как ФА,
...
#### Т.к. ФА так же может описывать множество ФА, как ФБО состоящее из УП типа (УП, ФА),
...
#### ФА может самоопределить себя одной УП
