...
В этой статье хотелось бы сравнить основные математические основы двух самых популярных теорий и ассоциативной теории.
...
Данные в реляционной модели группируются в отношения. Используя n-кортежи в реляционной модели, можно точно представить любую возможную структуру данных. Но не уверены, что вообще нужны n-кортежи. Например, каждый n-кортеж можно представить в виде вложенных упорядоченных пар (2-кортежей). Также не часто видят, что столбцы в таблицах используются в качестве индексов для элементов последовательности (n-кортеж). В некоторых базах данных даже запрещено использовать более $\mathbf{32}$ столбцов в таблице и ее строке (n-кортеж). Так что $\mathbf{N}$ обычно меньше $\mathbf{32}$. Так что в этом случае нет реальных n-кортежей, даже в современных реляционных базах данных.
...
Данные в модели графа представлены в виде коллекции узлов и ребер, и можно использовать эту модель практически для всего, кроме, возможно, последовательностей (n-кортежей). Ну, возможно, можно представить [последовательности в виде множеств](https://en.wikipedia.org/wiki/Tuple#Tuples_as_nested_sets), но на наш взгляд это запутанный способ представления последовательностей. Вероятно, не одни, и это может быть причиной, почему не видели, чтобы другие люди так делали.
...
Графическое представление сети дуплетов в виде графа.

![alt text](image-3.png)

Это графическое представление декартова произведения, которое представляет значения связей. Значения связей с оранжевыми скобками выбраны функцией $\mathbf{\lambda}$ в примере.
...
Графическое представление сети триплетов в виде графа.

Данные в сети триплетов представлены с использованием триплетов (3-кортежей).

Триплеты могут делать все то же, что и дуплеты, и особенно полезны, когда нужно указывать типы или значения непосредственно для каждой ссылки.
...
Последовательности по сути эквивалентны в выразительной силе с реляционной моделью. Но когда замечено, что дуплеты и триплеты достаточны для представления последовательностей любого размера, может быть, не нужно использовать сами последовательности напрямую.

## Заключение

Реляционная модель может представить все, включая ассоциативную модель. Графическая модель особенно хороша в представлении отношений и не так хороша в представлении последовательностей.

Ассоциативная модель может легко представить n-кортеж неограниченной длины с использованием кортежей с $\mathbf{n \geq 2}$, и она так же хороша, как теория графов, в своей способности представлять ассоциации, но она также мощна, как реляционная модель, и может полностью представить любую таблицу SQL.
